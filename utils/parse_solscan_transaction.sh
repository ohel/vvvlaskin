...
[ "$main_currency" = "ETH" ] && default_exchange="Uniswap"
...
fee_text="including" && [ "$main_currency" = "ETH" ] && fee_text="excluding"
echo -n "Enter amount of $currency_sold sold/swapped from (token balance change $fee_text fees): "
read s_amount
s_amount=$(echo $s_amount | tr -d -c '[0-9].')
echo -n "Enter amount of $currency_bought bought/swapped to (token balance change $fee_text fees): "
read b_amount
b_amount=$(echo $b_amount | tr -d -c '[0-9].')

if [ "$main_currency" = "ETH" ]
then
    echo -n "Enter transaction fee in ETH: "
    read vc_fee
    vc_fee=$(echo $vc_fee | tr -d -c '[0-9].')
    vc_text="\"vcfee\": $vc_fee, "
fi
...
then
    [ "$main_currency" = "ETH" ] && echo "Not implemented." && exit 1
